import { C, VISION, fmtUSD, FINANCING } from '../data/constants';
import { DonutChart } from '../components/Charts';
import { SectionTitle, StatPill, Card, DataTable } from '../components/UI';

export default function Financing() {
  const total = FINANCING.reduce((s, f) => s + f.usd, 0);
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "40px 24px", color: "#fff", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: C.soft, fontSize: 16, lineHeight: 1.7, maxWidth: 700, margin: 0 }}>
          Plan finansman solid $850M ki reparti sou 6 sous estratejik. Chak sous finansman konplète lòt pou asire dirab ak entegrite finansye a long tèm.
        </p>
      </div>
            ...
            <div key={f.source} style={{ marginBottom: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  <span style={{ color: C.light, fontWeight: 700, fontSize: 14 }}>{fmtUSD(f.usd)}</span>
                  <span style={{ color: C.soft, fontSize: 12 }}>{((f.usd / total) * 100).toFixed(1)}%</span>
                </div>
              </div>
              <div style={{ height: 8, background: "rgba(0,40,100,0.4)", borderRadius: 4, overflow: "hidden" }}>
                <div style={{
                  height: "100%",
                  width: `${(f.usd / total) * 100}%`,
                  background: f.color,
                  borderRadius: 4,
      ...
      <Card style={{ padding: 0, overflow: "hidden", marginBottom: 32 }}>
        <DataTable
          ...
          headers={["Sous Finansman", "Montant USD", "% Bidjè", "Kondisyon & Detay"]}
          rows={FINANCING.map(f => [
            <span style={{ fontWeight: 600 }}>{f.source}</span>,
            <span style={{ color: C.light, fontWeight: 700 }}>{fmtUSD(f.usd)}</span>,
            <span style={{ color: C.accent }}>{((f.usd / total) * 100).toFixed(1)}%</span>,
            f.detail,
          ])}
          footer={[
            <span style={{ fontWeight: 800 }}>TOTAL</span>,
            <span style={{ color: C.light, fontWeight: 800 }}>{fmtUSD(total)}</span>,
            "100%",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16, marginBottom: 32 }}>
        {[
          ...
          { icon: "💹", title: "Couverture Deviz", desc: "Estrateji pwoteksyon kont fliktiyasyon monè HTG/USD" },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
        {[
          { yr: "2025–26", label: "Mobilizasyon", amt: "$200M", pct: "24%", desc: "Gouvènman + BID pèmye tranch" },
          { yr: "2026–27", label: "Konstriksyon Faz 1", amt: "$180M", pct: "21%", desc: "Ijans, ICU, Sal Op, Labo" },
          { yr: "2027–28", label: "Espesyalite", amt: "$280M", pct: "33%", desc: "Matènite, Kardi, Nèwoloji" },
          { yr: "2028–29", label: "Teknoloji", amt: "$230M", pct: "27%", desc: "IRM, AI, Sant Kansè, Robotik" },
          { yr: "2029–31", label: "Ekspansyon", amt: "$160M", pct: "19%", desc: "Klinik satelit, Fòmasyon final" },
        ].map((t) => (
